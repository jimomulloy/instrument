...
public class PeakProcessor {
	...
	public PeakProcessor(ToneTimeFrame timeFrame) {
		...
		int i = 0;
		for (ToneMapElement element : timeFrame.getElements()) {
			this.magnitudes[i] = (float) element.amplitude;
			i++;
		}
	...
	public static float[] calculateNoiseFloor(float[] magnitudes,
		...
		// For each element take a median of surrounding values
		// (noiseFloorBuffer)
	...
	public static List<SpectralPeak> findPeaks(float[] magnitudes,
		...
		// filter the local maxima indexes, remove peaks that are too close to
		// each other

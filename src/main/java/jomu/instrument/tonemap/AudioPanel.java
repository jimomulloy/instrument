 ...
 * Audio SubSystem of the ToneMap 
 *
...
public class AudioPanel extends JPanel implements ToneMapConstants{
	...
	class ResolutionControl extends JPanel {
		...
		public ResolutionControl() {
			...
			resolutionSlider = new TmSlider(JSlider.HORIZONTAL, 1, 10, 1,
	...
	class PowerControl extends JPanel {
		...
		public PowerControl() {
			...
			lowPTS = new TmSlider(JSlider.HORIZONTAL, 0, 100, 0,
									"Low Threshold", new PowerSListener());
			add(lowPTS);
			
			highPTS = new TmSlider(JSlider.HORIZONTAL, 0, 100, 100,
												"High Threshold", new PowerSListener());
			add(highPTS);
	...
	class OscillatorControl extends JPanel {
		...
		public OscillatorControl() {
			...
			osc1S = new TmSlider(JSlider.HORIZONTAL, 0, 100, 0,
									"Oscillator 1", new OscSListener());
			add(osc1S);
			
			osc2S = new TmSlider(JSlider.HORIZONTAL, 0, 100, 0,
												"Oscillator 2", new OscSListener());
			add(osc2S);
	...
	class POffsetControl extends JPanel {
			...
			public POffsetControl() {
				...
				pOffsetSlider = new TmSlider(JSlider.HORIZONTAL, 0, 100, 0,
										"Pitch Offset", new POffsetListener());
				add(pOffsetSlider);
	...
	class PFactorControl extends JPanel {
		...
		public PFactorControl() {
			...
			pFactorSlider = new TmSlider(JSlider.HORIZONTAL, 0, 100, 100,
	...
	class TFactorControl extends JPanel {
		...
		public TFactorControl() {
			...
			tFactorSlider = new TmSlider(JSlider.HORIZONTAL, 0, 100, 80,
	...
	class ReverbSControl extends JPanel {
		...
		public ReverbSControl() {
			setLayout(new BoxLayout(this, BoxLayout.X_AXIS));
			reverbSSlider = new TmSlider(JSlider.HORIZONTAL, 0, 100, 0, 
									"Reverb Send",new ReverbSSliderListener());
			add(reverbSSlider);			
		}
	...
	class ReverbRControl extends JPanel {
		...
		public ReverbRControl() {
			setLayout(new BoxLayout(this, BoxLayout.X_AXIS));
			reverbRSlider = new TmSlider(JSlider.HORIZONTAL, 0, 100, 0, 
									"Reverb Return",new ReverbRSliderListener());
			add(reverbRSlider);			
		}
	   		...
	   		fc.setFileFilter(new javax.swing.filechooser.FileFilter () {
	 	        	public boolean accept(File f) {
	   	            	if (f.isDirectory()) {
	       	  			...
	       	  			if (name.endsWith(".au") || name.endsWith(".wav") || name.endsWith(".WAV") 
					...
					File file = null;
	 	         	JFileChooser fc = new JFileChooser(audioModel.toneMapFrame.getDirectory());
		            	...
		            	if (fc.showOpenDialog(audioModel.toneMapFrame.mainFrame) == JFileChooser.APPROVE_OPTION) {
	 	                	file = fc.getSelectedFile();
	 	                	audioModel.toneMapFrame.setDirectory(file);
	 	                	if (!audioModel.load(file)) return;
	...
	class FilterControl extends JPanel {
		...
		public FilterControl() {
			...
			t1S = new TmSlider(JSlider.HORIZONTAL, 0, 100, 50,
									"t1", new FilterSListener());
			add(t1S);
			...
			t2S = new TmSlider(JSlider.HORIZONTAL, 0, 100, 50,
												"t2", new FilterSListener());
			add(t2S);
			...
			t3S = new TmSlider(JSlider.HORIZONTAL, 0, 100, 50,
													"t3", new FilterSListener());
			add(t3S);
			...
			t4S = new TmSlider(JSlider.HORIZONTAL, 0, 100, 50,
										"t4", new FilterSListener());
			add(t4S);

...
/**
  * This class encapsulates a set of data objects that define the internal Tone "Map" 
  * This includes a collection of ToneMapElements organised within a "Matrix" of 2 dimensions 
  * Time and Pitch coordinates as defined in the TimeSest and PitchSet objects.
  * This class provides methods for accessing the Map as a whole and also provides an inner class
  * for iterating through elements of the Map Matrix.
  *
...
public class ToneMapMatrix implements Serializable {
	...
	public double FTPowerToAmp(double FTPower) {
		...
		if (ampType == LOGAMP) {
			if (minFTPower < maxFTPower/(double)lowThres*10.0) minFTPower = maxFTPower/(double)lowThres*10.0; //??		
			...
			if ((getMaxFTPower()-getMinFTPower())<= 0){
				...
				double maxpow = getMaxFTPower() -(getMaxFTPower()-getMinFTPower())*((double)(100-highThres)/100.0);
				if (FTPower > maxpow) {
					amplitude = 1.0;

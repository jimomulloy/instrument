...
/**
 * This class handles the User Interface functions for the central ToneMap
 * control class. This includes inner clases to handle display and control of a
 * Scrollable View pane that displays the contents of the ToneMap Matrix
 * objects.
 *
...
public class ToneMapPanel extends JPanel implements ProgressListener, ToneMapConstants {

	class ActionControl extends JPanel {

		class ActionCBListener implements ItemListener {
			...
			public void itemStateChanged(ItemEvent e) {
				if (e.getSource() instanceof JComboBox) {
					...
					if (name.startsWith("Audio")) {
						toneMap.audioSwitch = cb.isSelected();
					} else if (name.startsWith("Midi")) {
						toneMap.midiSwitch = cb.isSelected();
					}
				}
			}
		}
		...
		ActionControl() {
			...
			audioCB = new JCheckBox("Audio");
			audioCB.addItemListener(new ActionCBListener());
			add(audioCB);

			midiCB = new JCheckBox("Midi");
			midiCB.addItemListener(new ActionCBListener());
			add(midiCB);
	...
	class ScaleControl extends JPanel {
		...
		ScaleControl() {
			...
			timeScaleSlider = new TmSlider(JSlider.HORIZONTAL, 1, 100, INIT_TIME_SCALE, "Time Scale",
					new ScaleSliderListener());
			pitchScaleSlider = new TmSlider(JSlider.HORIZONTAL, 1, 100, INIT_PITCH_SCALE, "Pitch Scale",
					new ScaleSliderListener());
			add(timeScaleSlider);
	...
	class ThreshholdControl extends JPanel {
		...
		class ThreshholdSliderListener implements ChangeListener {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (true) {
					...
					toneMapView.coordinate();
					toneMap.setThreshhold(lowThreshhold, highThreshhold);
					slider.repaint();
		...
		ThreshholdControl() {
			setLayout(new BoxLayout(this, BoxLayout.X_AXIS));
			lowThreshholdSlider = new TmSlider(JSlider.HORIZONTAL, 0, 100, INIT_LOW_THRESHHOLD, "Low",
					new ThreshholdSliderListener());
			highThreshholdSlider = new TmSlider(JSlider.HORIZONTAL, 0, 100, INIT_HIGH_THRESHHOLD, "High",
					new ThreshholdSliderListener());
	...
	class ToneMapView extends JPanel implements Scrollable {
		...
		public void coordinate() {
			...
			if (toneMapMatrix != null) {
				...
				timeRule.setLimits(0, toneMap.getTimeSet().getEndTime() - toneMap.getTimeSet().getStartTime());

				pitchRule.setLimits(toneMap.getPitchSet().getLowNote(), toneMap.getPitchSet().getHighNote());
				...
				do {
					...
					do {
						...
						if (toneMapElement != null) {
							...
							g.fillRect(timeCoords[x], pitchCoords[y] - pitchStep, timeStep, pitchStep);
							g.drawRect(timeCoords[x], pitchCoords[y] - pitchStep, timeStep, pitchStep);
						}
	...
	class ToneMapViewer extends JPanel {
		...
		public ToneMapViewer() {
			...
			viewScrollPane = new JScrollPane(toneMapView, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
			...
			viewScrollPane.setCorner(JScrollPane.UPPER_LEFT_CORNER, new Corner());
			viewScrollPane.setCorner(JScrollPane.LOWER_LEFT_CORNER, new Corner());
			viewScrollPane.setCorner(JScrollPane.UPPER_RIGHT_CORNER, new Corner());
	...
	public TmSlider lowThreshholdSlider;

	public TmSlider highThreshholdSlider;
	...
	private JCheckBox audioCB, midiCB;
	...
	public int lowThreshhold = INIT_LOW_THRESHHOLD;

	public int highThreshhold = INIT_HIGH_THRESHHOLD;

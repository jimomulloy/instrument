...
import jomu.instrument.tonemap.filters.IIRBandpassFilter;
import jomu.instrument.tonemap.filters.IIRBandpassFilterDesign;
import jomu.instrument.tonemap.filters.IIRLowpassFilter;
import jomu.instrument.tonemap.filters.IIRLowpassFilterDesign;
...
public final class Wavelet {
	...
	public static final void convert(double[] wave, double[] s, double[] freq, double dt, int skip, int mm, int nn,
			ProgressListener progressListener, double pFactor, double tFactor, double pOffset, double t1Setting,
			double t2Setting, double t3Setting, double t4Setting, boolean t1Switch, boolean t2Switch, boolean t3Switch,
			boolean t4Switch

	) {

		int i, j, index = 0, index1 = 0, ii = 0, lmin = 0, lmax = 0, offset = 0, offsetmin = 0, offsetmax = 0, step = 0,
				steplast = 0;
		...
		for (i = nn - 1; i >= 0; i--) {
			...
			if (t1Switch) {
				if (firstflag == 0) {
					lpfd = new IIRLowpassFilterDesign((int) (k * (double) t2Setting / (double) t3Setting),
							(int) (1.0 / dt), (double) t1Setting);
					lpfd.doFilterDesign();
				...
				for (j = 0; j < mm; j++) {
					...
					if (j % skip == 0) {
						...
						for (ii = lmin; ii <= lmax; ii += step) {
							/*
							 * integrate s*conj(psi) - note s is always real. We can speed things up a bit
							 * if we calculate the terms used in the real and imaginary parts of ctot in one
							 * go... not very neat though
							 */
							tval = ii * dt;
							...
							if (ii >= 0 && ii < mm) {
								if (t1Switch) {
									t1 = filterBuffer2[ii]
											* Math.exp(-2.0 * k * (tval - t) * k * (tval - t) * pi * pi / (z0 * z0));
								} else {
									t1 = filterBuffer1[ii]
											* Math.exp(-2.0 * k * (tval - t) * k * (tval - t) * pi * pi / (z0 * z0));
								}
								...
								t2 = Math.exp(
										(-z0 * z0 / 2.0) - 2 * k * (tval - t) * k * (tval - t) * pi * pi / (z0 * z0));
					...
					for (j = 0; j < mm; j++) {
						...
						if (j % skip == 0) {
							...
							for (ii = lmin; ii <= lmax; ii += step) {
								...
								if (ii >= 0 && ii < mm) {
									if (t1Switch) {
										t1 = filterBuffer3[ii] * Math
												.exp(-2.0 * k * (tval - t) * k * (tval - t) * pi * pi / (z0 * z0));
									} else {
										t1 = filterBuffer1[ii] * Math
												.exp(-2.0 * k * (tval - t) * k * (tval - t) * pi * pi / (z0 * z0));
									}

									t2 = Math.exp((-z0 * z0 / 2.0)
											- 2 * k * (tval - t) * k * (tval - t) * pi * pi / (z0 * z0));
									ctotr = ctotr + (Math.sin(2.0 * pi * k * (tval - t)) * t1 - t2);
									ctoti = ctoti + (Math.cos(2.0 * pi * k * (tval - t)) * t1 - t2);
								} else {
									t1 = 0;

 ...
 * Midi Sub system of the ToneMap 
 *
...
public class MidiPanel extends JPanel implements ToneMapConstants {
	...
	class ChannelControl extends JPanel {
		...
		public ChannelControl() {
			...
			veloS = new TmSlider(JSlider.HORIZONTAL, 0, 127,
	            		INIT_VELOCITY_SETTING, 
						"Velocity",new ControlSliderListener());
			presS = new TmSlider(JSlider.HORIZONTAL, 0, 127,
						INIT_PRESSURE_SETTING, 
						"Pressure",new ControlSliderListener());
			revbS = new TmSlider(JSlider.HORIZONTAL, 0, 127,
						INIT_REVERB_SETTING, 
						"Reverb",new ControlSliderListener());
			bendS = new TmSlider(JSlider.HORIZONTAL, 0, 16383,
						INIT_BEND_SETTING, 
	...
	public boolean save() {
		...
		try {
	        ...
	        if (fc.showSaveDialog(midiModel.toneMapFrame.mainFrame) == JFileChooser.APPROVE_OPTION) {
		      	file = fc.getSelectedFile();   	
	     		if (!midiModel.saveMidiFile(file)) return false;

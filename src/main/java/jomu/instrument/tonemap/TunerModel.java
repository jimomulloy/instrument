...
/**
  * This class defines the Tuner Sub System Data Model processing 
  * functions for the ToneMap including execution of ToneMapMatrix data
  * filtering and conversion, generation of NoteList objects of MIDI notes
  * and Control Settings management through the TunerPanel class.
  *
...
public class TunerModel implements ToneMapConstants {
	...
	/**
 	* TunerModel constructor.
 	* Instantiate TunerPanel
 	*/
	...
	*/
	...
	/**
 	* Execute Tuner processing function on current ToneMap objects
 	* Involves filtering and conversion of data to produce NoteList object
 	* of MIDI note data  
 	*/
	...
	* Apply filtering and conversion processing on basis of Tuner Parameters
	...
	private boolean noteScan() {
		...
		//Iterate through ToneMapMatrix processing data elements to derive NoteList elements
	...
	private void attenuate(ToneMapElement overToneElement, double fundamental, double harmonic) {
		...
		//overToneElement.postAmplitude = (overToneElement.postFTPower-toneMapMatrix.getMinFTPower()) / 
	...
	/**
 	* Scan through ToneMapMatrix extracting MIDI beats of dominant rythmic note data 
 	*/
	private boolean beatScan() {
		...
		do {
			...
			do {
				...
				amplitude = mapIterator.getElement().postAmplitude;
				if (amplitude > (double)noteHigh/100.0) amplitude = (double)noteHigh/100.0;
				ampSum = ampSum + amplitude;
	...
	private boolean normalize() {
		...
		do {
			...
			do {
				...
				note = pitchSet.getNote(mapIterator.getPitchIndex());
				amplitude = mapIterator.getElement().postAmplitude;
				if (amplitude > lastAmp) startPeak = index;
				if (amplitude >= lastAmp) endPeak = index;
				...
				if (amplitude < troughAmp) troughAmp = amplitude;
				lastAmp = amplitude;
	...
	private void processPeak(int startPeak,int endPeak, double trough, int thresholdIndex ) {
		...
		do {
			...
			if (amplitude >= normalThreshold(index, thresholdIndex) && amplitude > ampThres) {
				...
				peakIterator.getElement().postAmplitude = 
							toneMapMatrix.FTPowerToAmp(peakIterator.getElement().postFTPower);
	...
	private double normalThreshold(int normalIndex, int thresholdIndex) {
		...
		double threshold = 0;
		if (n2Switch) {
			threshold = ((double)noteHigh/100.0)*(thresholdAmp)/((double)n5Setting/10.0 + (((double)n4Setting/(double)normalizeSetting)*(double)(normalNote-thresholdNote)));
		} else {
			threshold = ((double)noteHigh/100.0)*(thresholdAmp)/((double)n5Setting/10.0 + (((double)n4Setting/(double)normalizeSetting)*(double)(normalFreq-thresholdFreq)));
		}

		System.out.println("In normalThreshold: "+note+", "+threshold+", "+thresholdAmp+", "+thresholdFreq+", "+normalFreq);
		return threshold;
	}
	...
	private void applyFormant(ToneMapElement element, int note) {
		...
		if (formantMidFreq < formantLowFreq || formantMidFreq > formantHighFreq) return;
		
		double noteFreq = pitchSet.getMidiFreq(note);
		
		if (noteFreq < formantLowFreq || noteFreq > formantHighFreq) return;

		
		if (noteFreq <= formantMidFreq) {
			element.postFTPower = element.postFTPower 
					*(1.0-(((double)formantFactor/100.0)*((noteFreq-formantLowFreq)/(formantMidFreq-formantLowFreq))));
		
		} else {
			element.postFTPower = element.postFTPower 
					*(1.0-(((double)formantFactor/100.0)*((formantHighFreq-noteFreq)/(formantHighFreq-formantMidFreq))));

	...
	public void accept(List<NuMessage> messages) {
		...
		for (NuMessage message : messages) {
			...
			if (message.source.getCellType().equals(CellTypes.AUDIO_PRE_CHROMA)) {
				System.out.println(">>AudioChromaPostProcessor accept: " + message + ", streamId: " + streamId);
				...
				ToneTimeFrame postTimeFrame = preTimeFrame.clone().smoothMedian(preChromaToneMap, sequence);
				postChromaToneMap.addTimeFrame(postTimeFrame);
				if (sequence >= 10 && (sequence % 10 == 0)) {
					postTimeFrame.downSample(postChromaToneMap, 10, sequence);

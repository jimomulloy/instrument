...
public class AudioSpectrumProcessor implements Consumer<List<NuMessage>> {
	...
	public void accept(List<NuMessage> messages) {
		...
		for (NuMessage message : messages) {
			...
			System.out.println(">>AudioSpectrumProcessor accept: " + message + ", streamId: " + streamId);
			if (message.source.getCellType().equals(CellTypes.SOURCE)) {
				Hearing hearing = Instrument.getInstance().getCoordinator().getHearing();
				ToneMap toneMap = worldModel.getAtlas().getToneMap(buildToneMapKey(CellTypes.AUDIO_SPECTRUM, streamId));
				...
				if (afp != null) {
					...
					PitchAnalyser pitchAnalyser = new PitchAnalyser(fftSpectrum, toneMap.getTimeFrame().getPitches(),
							20);
					...
					PitchDetect2 pd = new PitchDetect2(1024,
							(float) toneMap.getTimeFrame().getTimeSet().getSampleRate(),

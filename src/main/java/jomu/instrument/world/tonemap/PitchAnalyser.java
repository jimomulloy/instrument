...
public class PitchAnalyser {
	...
	private ArrayList<Integer>[] f0indHarm; /* Klapuri F0 candidate indices harmonics */
		...
		for (int k = 0; k < f0index.length; ++k) {
			...
			for (int h = 0; h < harmonics; ++h) {
				ArrayList<Integer> tempInd = find(this.fftSpectrum.getBinFrequencies(),
						f0cands[k] * ((double) h + 1.0) - halfBinWidth, f0cands[k] * ((double) h + 1.0) + halfBinWidth);
	...
	public Vector<Double> detectF0s() {
		...
		Vector<ArrayList> F0BinIndexes = new Vector<ArrayList>();

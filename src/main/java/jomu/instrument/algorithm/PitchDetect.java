package jomu.instrument.algorithm;
    ...
    private void detectfzero(float[] spec, float[] fzeroInfo)
    {
        ...
        for (int j = 0; j < PITCHES.length; ++j) {
            ...
            for (int i = 1; i * PITCHES[j] < sampleRate / 2; ++i) {
                ...
                if (bin == timeSize/2) val = Math.max(spec[bin-3], Math.max(spec[bin-2], spec[bin-1]));
                else if (bin == timeSize/2-1) val = Math.max(spec[bin-3], Math.max(spec[bin-2], Math.max(spec[bin-1], spec[bin])));
                else val = Math.max(spec[bin-3], Math.max(spec[bin-2], Math.max(spec[bin-1], Math.max(spec[bin], spec[bin+1]))));
...
final class SpecWhitener
{
    ...
    public SpecWhitener(int bufferSize, float sr)
    {
        ...
        for (int i = 0; i < cenFreqs.length; ++i) cenFreqs[i] = (float) (229 * (Math.pow(10, (i + 1) / 21.4f) - 1));

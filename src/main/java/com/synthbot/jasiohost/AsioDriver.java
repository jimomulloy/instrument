...
/**
 * The <code>AsioDriver</code> class represents an ASIO driver. Only one may be
 * loaded at any time. A new driver is instantiated with
 * <code>getDriver()</code>, supplied with the name of the driver to load, as
 * derived from <code>getDriverNames()</code>.<br>
 * <br>
 * All methods may throw an <code>AsioException</code>, which is a
 * <code>RuntimeException</code> (i.e., it is not required to be caught). The
 * error message will have some information about what has gone wrong.
 * Generally, it is not possible to recover from such errors, except for
 * shutting down the driver.<br>
 * <br>
 * Note that most methods also throw an <code>IllegalStateException</code> if
 * the driver is not in the appropriate state for that method to be called. For
 * instance, if <code>start()</code> is called before
 * <code>createBuffers()</code>, an <code>IllegalStateException</code> will be
 * thrown.<br>
 * <br>
 * After the driver has been instantiated with <code>getDriver()</code>, the
 * sequence of setup calls are: <code>createBuffers()</code> and then
 * <code>start()</code>. Once the driver has been started,
 * <code>bufferSwitch()</code> will be called on any
 * <code>AsioDriverListener</code>s and audio can be sent or received from the
 * driver. In order to shut down the driver from the <code>RUNNING</code> state,
 * call: <code>stop()</code>, <code>disposeBuffers()</code>,
 * <code>exit()</code>, and finally <code>unloadDriver()</code> (if the driver
 * should be fully unloaded from memory). Alternatively
 * <code>returnToState()</code> can also be used in order to return the
 * <code>AsioDriver</code> to a particular <code>AsioDriverState</code>.
 */
public class AsioDriver {
	...
	/**
	 * The designated ASIO driver is loaded and returned in the
	 * <code>INITIALIZED</code> state. If a driver is already loaded and it is the
	 * named driver, then that driver object is returned. If the named driver is
	 * different from the one currently loaded, then the currently loaded driver is
	 * shut down and a new one is instantiated. There is only ever one loaded
	 * driver.
	 * 
	 * @param driverName The name of the driver to load, as returned by
	 *                   <code>getDriverNames()</code>.
	 * @return The named AsioDriver object.
	 * @throws AsioException        Thrown if the driver could not be loaded into
	 *                              memory.
	...
	/**
	 * Due to the way in which ASIO drivers are integrated into Windows (as COM
	 * objects), it is necessary to register each thread which accesses any ASIO
	 * methods. Such registration is done automatically for the thread which loads
	 * the <code>AsioDriver</code> class (usually the first thread to access any
	 * <code>AsioDriver</code> method). Failure to register additional threads may
	 * cause exceptions, non-function of methods, or other arbitrary behaviour. A
	 * thread may register itself more than once, but this is not necessary.
	 * Registration is lightweight.
	 */
	private static native void registerThread();

	/**
	 * Returns the currently loaded <code>AsioDriver</code>. If no driver is loaded,
	 * <code>null</code> is returned. Check to see if a driver is currently loaded
	 * with <code>isDriverLoaded()</code>.
	 */
	...
	static {
		// load jasiohost32.dll or jasiohost64.dll depending on the the bit-depth of the
		// JVM
	...
	/**
	 * Returns <code>true</code> if a driver is currently loaded, <code>false</code>
	 * otherwise.
	 */
	...
	/**
	 * A list of all (maximum 32) ASIO drivers registered with the system is
	 * returned.
	 */
	...
	/*
	 * Normally the driver is shut down manually. But it is put into finalize() in
	 * order to properly shut down the driver in case the JVM is otherwise stopped.
	 * (non-Javadoc)
	 * 
	...
	/**
	 * Closes all data structures relating to the operation of the ASIO driver,
	 * though the driver remains loaded in memory. Returns the driver from the
	 * <code>INITIALIZED</code> state to the <code>LOADED</code> state.
	 */
	 ...
	 * A control panel may not be provided by all drivers on all platforms.
	 */
	public synchronized void openControlPanel() {
		if (currentState.ordinal() < AsioDriverState.INITIALIZED.ordinal()) {
			throw new IllegalStateException(
					"The AsioDriver must be at least in the INITIALIZED state: " + currentState.toString());
		}
		registerThreadIfNecessary();
		ASIOControlPanel();
	}
	...
	/**
	 * Returns the number of available input channels. -1 is returned if there is an
	 * error.
	 * 
	...
	/**
	 * Returns the number of available output channels. -1 is return if there is an
	 * error.
	 * 
	 ...
	 * external sync. The driver must be in the <code>INITIALIZED</code> state.
	 * 
	 * @param sampleRate The sample rate in Hz.
	 */
	public synchronized void setSampleRate(double sampleRate) {
		if (currentState.ordinal() != AsioDriverState.INITIALIZED.ordinal()) {
			throw new IllegalStateException(
					"The AsioDriver must be in the INITIALIZED state: " + currentState.toString());
		}
		registerThreadIfNecessary();
	...
	/**
	 * Returns the preferred buffer size. The host should attempt to use this buffer
	 * size.
	 */
	...
	/**
	 * Returns the granularity at which buffer sizes may differ. Usually, the buffer
	 * size will be a power of 2; in this case, granularity will be reported as -1,
	 * signaling possible buffer sizes starting from the minimum size, increased in
	 * powers of 2 up to maximum size.
	 */
	...
	/**
	 * Note: As <code>getLatencyInput()</code> will also have to include the audio
	 * buffer size of the <code>createBuffers()</code> call, the application should
	 * call this function after the buffer creation. In the case that the call
	 * occurs beforehand the driver should assume preferred buffer size.
	 * 
	...
	/**
	 * Note: As <code>getLatencyOutput()</code> will also have to include the audio
	 * buffer size of the <code>createBuffers()</code> call, the application should
	 * call this function after the buffer creation. In the case that the call
	 * occurs beforehand the driver should assume preferred buffer size.
	 * 
	...
	/**
	 * Get information about an input channel. The returned <code>AsioChannel</code>
	 * object is persistent for as long as the driver is <code>INITIALIZED</code>,
	 * and can be retrieved in any context.
	 * 
	 * @param index The input channel index to get information about.
	 * @return An <code>AsioChannel</code> object representing the requested input
	 *         channel.
	 * @throws IndexOutOfBoundsException Thrown if the requested channel index is
	 *                                   out of bounds. The channel does not exist.
	 */
	...
	/**
	 * Get information about an output channel. The returned
	 * <code>AsioChannel</code> object is persistent for as long as the driver is
	 * <code>INITIALIZED</code>, and can be retrieved in any context.
	 * 
	 * @param index The output channel index to get information about.
	 * @return An <code>AsioChannel</code> object representing the requested output
	 *         channel.
	 * @throws IndexOutOfBoundsException Thrown if the requested channel index is
	 *                                   out of bounds. The channel does not exist.
	 */
	...
	/**
	 * Creates audio buffers for the set of designated channels. The buffer size is
	 * that as returned by <code>getBufferPreferredSize()</code>.
	 * 
	 * @param channelsToInit A <code>Set</code> of <code>AsioChannelInfo</code>
	 *                       objects designating the input and output channels to
	 *                       initialise and create audio buffers for.
	 */
	public synchronized void createBuffers(Set<AsioChannel> channelsToInit) {
		if (!AsioDriverState.INITIALIZED.equals(currentState)) {
			throw new IllegalStateException(
					"The ASIO driver must be in the INITIALIZED state in order to createBuffers().");
		}
	...
	/**
	 * Remove the previously created audio buffers (with
	 * <code>createBuffers()</code>). The active channels are reset; all channels
	 * become inactive.
	 */
	public synchronized void disposeBuffers() {
		if (!AsioDriverState.PREPARED.equals(currentState)) {
			throw new IllegalStateException(
					"The ASIO driver must be in the PREPARED state " + "in order to dispose of the audio buffers.");
		}
	...
	/**
	 * Start the driver. Input buffers are presented and output buffers consumed via
	 * calls to <code>bufferSwitch()</code>. Puts the <code>AsioDriver</code> into
	...
	/**
	 * Stop the driver. Calls to <code>bufferSwitch()</code> will cease. The
	 * <code>AsioDriver</code> returns to the <code>PREPARED</code> state.
	 */
	...
	/**
	 * Shutdown the ASIO driver, regardless of what state it is in. Unload it from
	 * memory. This is a convenience method for
	 * <code>returnToState(AsioDriverState.UNLOADED)</code>.
	 */
	...
	/**
	 * Return the driver to a given state. If the target state is ahead or equal to
	 * the current state then this method has no effect.
	 * 
	...
	/**
	 * Add a new <code>AsioDriverListener</code>. Listeners can only be updated
	 * while the driver is in the LOADED or INITIALIZED state.
	 * 
	 * @param listener A new <code>AsioDriverListener</code> which should be sent
	 *                 callbacks.
	 */
	...
	/**
	 * Unregister an <code>AsioDriverListener</code>. Listeners can only be updated
	 * while the driver is in the LOADED or INITIALIZED state.
	 * 
	...
	private void fireBufferSwitch(long systemTime, long samplePosition, int bufferIndex) {
		for (AsioChannel channel : inputChannels) {
			if (channel.isActive())
				channel.setBufferIndex(bufferIndex);
		}
		for (AsioChannel channel : outputChannels) {
			if (channel.isActive())
				channel.setBufferIndex(bufferIndex);
		}
		// NOTE(mhroth): use a standard for loop in order to avoid implicitly creating
		// iterator objects

...
/**
 * The <code>AsioChannel</code> class represents an input or output channel
 * available form the ASIO driver. It provides information such as current state
 * and sample type. This class also encapsulates and makes available native
 * audio buffers if it is active. Convenience methods are also available to
 * facilitate consuming audio.
 */
public class AsioChannel {
	...
	/**
	 * Returns the current buffer to read or write from, with the position reset to
	 * zero. The endian-ness of the buffer and of the underlying system has been
	 * accounted for. Note that input buffers <code>isInput()</code> are read-only.
	 */
	...
	/**
	 * A convenience method to read samples from the input buffer to a
	 * <code>float</code> array. The argument array must have the same length as the
	 * configured buffer size. The returned samples are bounded to within [-1,1].
	 * The <code>input</code> array should be the same size as the input array. If
	 * it larger, then a <code>BufferUnderflowException</code> will be thrown. If it
	 * is smaller, then the buffer will be incompletely read.
	 * 
	...
	public void read(float[] input) {
		...
		case ASIOSTInt24MSB: {
			for (int i = 0; i < input.length; i++) {
				int sampleValueInt = ((int) inputBuffer.get()) & 0xFFFF;
				sampleValueInt <<= 8;
	...
	/**
	 * Returns a string description of the channel in the format, "Output Channel 0:
	 * Analog Out 1/2 Delta-AP [1], ASIOSTInt32LSB, group 0, inactive"
	 */
	@Override
	public String toString() {
		...
		sb.append(name);
		sb.append(", ");
		sb.append(sampleType.toString());
		sb.append(", group ");
		sb.append(Integer.toString(channelGroup));
		sb.append(", ");

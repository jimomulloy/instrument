...
/**
 * This interface defines the callbacks that an ASIO driver may make to the
 * host.
 */
public interface AsioDriverListener {
	 ...
	 * reconfiguration. As this request is being made in a callback, the driver
	 * should only be reset after this callback method has returned. The recommended
	 * way to reset the driver is:
	 * 
	 * <pre>
	 * <code>
	 ...
	 * </pre>
	 * 
	 * Because all methods are synchronized, this approach will safely return the
	 * driver to the <code>INITIALIZED</code> state as soon as possible. The buffers
	 * must then be recreated and the driver restarted.
	 */
	public void resetRequest();

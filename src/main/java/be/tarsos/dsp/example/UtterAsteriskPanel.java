...
public class UtterAsteriskPanel extends JPanel {
	...
	double[] pattern = {400, 400, 600, 400, 900, 800, 400, 400, 600, 400, 1100,
			900}; // in cents
	...
	public void paint(final Graphics g) {
		...
		double lengthPerQuarterNote = patternLength
				/ patternLengthInQuarterNotes; // in seconds per quarter note
		...
		for (int i = 0; i < pattern.length; i++) {
			...
			int patternWidth = (int) (lengthInSeconds / (double) patternLength
					* getWidth());// pixels
			int patternHeight = (int) (CENTS_DEVIATION / 1200.0 * getHeight());
			int patternX = (int) ((currentXPosition) / (double) patternLength
					* getWidth());
			int patternY = getHeight()
					- (int) (pattern[i] / 1200.0 * getHeight())
					- patternHeight / 2;
		...
		for (int i = 0; i < pitches.size(); i++) {
			...
			int patternX = (int) (startTimeStamp / (double) patternLength
					* getWidth());
			int patternY = getHeight()
					- (int) (pitchInCents / 1200.0 * getHeight());
	...
	private void score() {
		...
		for (int i = 0; i < pitches.size(); i++) {
			double pitchInCents = pitches.get(i);
			double startTimeStamp = startTimeStamps.get(i) % patternLength;
			if (startTimeStamp > 0.5
					&& startTimeStamp <= 0.5 + 0.5 * pattern.length) {
				double lengthPerQuarterNote = patternLength
						/ patternLengthInQuarterNotes; // in seconds per quarter
														// note
				double currentXPosition = 0.5; // seconds of pause before start
				for (int j = 0; j < pattern.length; j++) {
					double lengthInSeconds = timing[j] * lengthPerQuarterNote;// seconds
					if (startTimeStamp > currentXPosition
							&& startTimeStamp <= currentXPosition
									+ lengthInSeconds
							&& Math.abs(pitchInCents
									- pattern[j]) < CENTS_DEVIATION) {
						score++;
					}
					currentXPosition += lengthInSeconds; // in seconds
				}

...
public class KernelDensityEstimate {
	 ...
	 * 
	 * @param shift
	 *            The number of positions the accumulator should be shifted.
	 */
	 ...
	 * @return A new KDE with the contents of the original mapped to the new
	 *         size.
	 */
	...
	/**
	 * Sets the area under the curve to 1.0. In essence every value is divided
	 * by getSumFreq(). As per definition of a probability density function.
	 */
	public void pdfify() {
	 ...
	 * 
	 * @param other
	 *            The other kde of the same size.
	 */
	 ...
	 *         are. 1 means total correlation, 0 no correlation.
	 */
	public double optimalCorrelation(final KDECorrelation correlationMeasure,
	 ...
	 *         correlation.
	 */
	public int shiftForOptimalCorrelation(
			final KDECorrelation correlationMeasure,
			final KernelDensityEstimate other) {
		int optimalShift = 0; // displacement with best correlation
		double maximumCorrelation = -1; // best found correlation

		for (int shift = 0; shift < size(); shift++) {
			final double currentCorrelation = correlationMeasure
					.correlation(this, other, shift);
			if (maximumCorrelation < currentCorrelation) {
				maximumCorrelation = currentCorrelation;
				optimalShift = shift;
			}
	...
	public static interface KDECorrelation {
		public double correlation(KernelDensityEstimate first,
				KernelDensityEstimate other, int shift);
	}
	...
	public static class Overlap implements KDECorrelation {
		public double correlation(KernelDensityEstimate first,
			...
			for (int i = 0; i < first.size(); i++) {
				...
				matchingArea += Math.min(first.getValue(i),
						other.getValue(otherIndex));
			}
			double biggestKDEArea = Math.max(first.getSumFreq(),
					other.getSumFreq());
			correlation = matchingArea / biggestKDEArea;
			return correlation;
		}
	...
	public static class Cosine implements KDECorrelation {
		public double correlation(KernelDensityEstimate first,
			...
			correlation = innerProduct / (Math.pow(firstSquaredSum, 0.5)
					* Math.pow(otherSquaredSum, 0.5));
			return correlation;
		}
	}
}

...
public class FFTComputationWrapper {
	 ...
	 * @param startTimeMs      timestamp in the original audio file at which this
	 ...
	 * @return a single FFTFrame that is the result of an FFT being computed on wave
	...
	public static FFTFrame doFFT(int[] wave, double startTimeMs, double windowDurationMs, double fileDurationMs,
		...
		double endMs = Math.min(fileDurationMs, startTimeMs + windowDurationMs); // last window(s) will probably be
																					// partial
	 ...
	 * scale
	 * 
	 * @param fftFrame single FFT frame for which amplitudes should be scaled
	 *                 logarithmically
	 */
	public static void scaleLogarithmically(FFTFrame fftFrame) {
		// dB is a measure that compares an intensity (amplitude) to some reference
		// intensity.
		// This reference intensity should be the maximum possible intensity for any
		// sample in the entire signal.

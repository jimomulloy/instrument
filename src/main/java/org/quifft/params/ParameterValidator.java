...
/**
 * Validates {@link FFTParameters} prior to the computation of FFT and throws a
 * {@link BadParametersException} if any invalid parameters are found
 */
	...
	public static void validateFFTParameters(FFTParameters params, boolean isFFTStream) {
		...
		if (params.windowSize <= 0)
			throw new BadParametersException(
					String.format("Window size must be positive; " + "was set to %d", params.windowSize));
		...
		if (params.windowOverlap < 0 || params.windowOverlap >= 1)
			throw new BadParametersException(
					String.format("Window overlap must be a positive value " + "between 0 and 0.99; was set to %f",
							params.windowOverlap));

		// num points, if set, must be positive
		if (params.numPoints != null && params.numPoints < 0)
			throw new BadParametersException(
					String.format("Number of points in FFT must be positive; " + "was set to %d", params.numPoints));
		...
		if (params.numPoints != null && params.numPoints < params.windowSize)
			throw new BadParametersException(String.format(
					"Number of points in FFT must be at least as large as "
		...
		if (params.numPoints != null && !isPow2(params.numPoints))
			throw new BadParametersException(String
					.format("Number of points in FFT must be a power of two; " + "was set to %d", params.numPoints));

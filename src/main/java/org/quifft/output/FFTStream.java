...
/**
 * FFTStream computes an FFT on an audio file incrementally as opposed to all at
 * once
 * <p>
 * It exposes an Iterator interface for computing {@link FFTFrame}s one at a
 * time. This can be a useful alternative to {@link FFTResult} if your audio
 * file is large or you are space-constrained.
 * </p>
 */
public class FFTStream extends FFTOutputObject implements Iterator<FFTFrame> {
	...
	// Counter for how many samples have been computed so far (how many times next()
	// has been called)
	...
	public FFTFrame next() {
		...
		FFTFrame nextFrame = FFTComputationWrapper.doFFT(nextWindow, startTimeMs, windowDurationMs, fileDurationMs,
				sampleRate, fftParameters);

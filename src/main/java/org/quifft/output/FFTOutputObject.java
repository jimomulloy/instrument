...
/**
 * Object representing the result of a Fourier transform; superclass of two
 * result types {@link FFTResult} and {@link FFTStream}
 */
public abstract class FFTOutputObject {
	 ...
	 * <p>
	 * This is calculated by dividing the audio file's sampling rate by the number
	 * of points in the FFT. A lower frequency resolution makes it easier to
	 * distinguish between frequencies that are close together. By improving
	 * frequency resolution, however, some time resolution is lost because there are
	 * fewer FFT calculations per unit of time (sampling windows are larger).
	 * </p>
	 */
	 ...
	 * <p>
	 * This is proportional to the length of each window in terms of number of
	 * samples.
	 * </p>
	 */
	...
	/**
	 * Sets metadata to be returned by an output object ({@link FFTResult} or
	 * {@link FFTStream})
	 * 
	...
	public String toString() {
		...
		if (fftParameters.windowOverlap == 0) {
			...
			builder.append(String.format("%s", decimalFormat.format(fftParameters.windowOverlap * 100))).append("%\n");
		}
		builder.append(String.format("Number of points in FFT: %d", fftParameters.windowSize));
		if (fftParameters.numPoints != null) {
			builder.append(String.format(" window size + %d zero-padding = %d", fftParameters.zeroPadLength(),
					fftParameters.numPoints));
		}

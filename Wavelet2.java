...
public final class Wavelet2 {
	...
	public static final void convert(	double[] wave,
		...
		for (i=nn-1; i>=0; i--) {
			...
			if (firstflag==0) {				
				lpfd = new IIRLowpassFilterDesign((int)(k*1.5), (int)(1.0/dt), 10);
				lpfd.doFilterDesign();
				lowPassShelf = new IIRLowpassFilter(lpfd);
				lowPassShelf.doFilterNoSum(filterBuffer1, filterBuffer2, mm);
			}
			
			bpfd = new IIRBandpassFilterDesign((int)(k), (int)(1.0/dt), 50.0);
			bpfd.doFilterDesign();
			...
			//	for (j=offset; j<(mm-offset); j++) {
				for (j=0; j<mm; j++) {
					...
					if (j % skip == 0) {
						...
						wave[index]+=0.5*(step*Math.sqrt(k)*Math.sqrt(ctotr*ctotr+ctoti*ctoti));
						if (index1 > 0) {
							wave[index1]+=0.5*(step*Math.sqrt(k)*Math.sqrt(ctotr*ctotr+ctoti*ctoti));
						}

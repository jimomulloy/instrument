...
public class EventBridgeRouting extends Construct {
	...
	public EventBridgeRouting(Construct scope, LogGroup logGroup, Function function) {
		...
		var rule = Rule.Builder.create(this, "BucketPutRule").ruleName("s3-put-eventbridge")
				.description("jomu.instrument").targets(List.of(cloudWatchTarget, functionTarget))
				.eventPattern(EventPattern.builder().source(List.of("aws.s3")).build()).build();

	...
	private class MidiQueueConsumer implements Runnable {
		...
		private boolean playVoiceChannel2(MidiQueueMessage mqm) {
			...
			if (midiSynthTracksSwitch) {
				...
				toneMap = workspace.getAtlas().getToneMap(ToneMap
						.buildToneMapKey(CellTypes.AUDIO_INTEGRATE.toString() + "_PEAKS", midiStream.getStreamId()));
			}
			toneTimeFrame = toneMap.getTimeFrame(mqm.sequence);
			if (toneTimeFrame == null) {
				return false;
				...
				for (ToneMapElement toneMapElement : ttfElements) {
					...
					if (noteListElement != null) {
						volume = getNoteVolume(lowVoiceThreshold, highVoiceThreshold, playLog, logFactor, playPeaks,
								toneMapElement.isPeak, noteListElement.maxAmp);
						NoteTrack track = toneMap.getNoteTracker().getTrack(noteListElement);
						if (track == null || track.getNumber() < 2 || ((track.getNumber() - 2) % 4 != 0)) {
							volume = 0;
						}
					}
					...
					if (volume > 0) {
						if (!voiceChannel2LastNotes.contains(note)) {
							...
							try {
								...
								if (writeTrack) {
									createEvent(voice1Track, voice2Channel, NOTEON, note, tick, volume);
								}
							} catch (InvalidMidiDataException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
						...
						if (voiceChannel2LastNotes.contains(note)) {
							...
							try {
								...
								if (writeTrack) {
									createEvent(voice1Track, voice2Channel, NOTEOFF, note, tick, volume);
								}
							} catch (InvalidMidiDataException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
				...
				for (ToneMapElement toneMapElement : ttfElements) {
					...
					if (noteListElement != null) {
						volume = getNoteVolume(lowVoiceThreshold, highVoiceThreshold, playLog, logFactor, playPeaks,
								toneMapElement.isPeak, noteListElement.maxAmp);
						NoteTrack track = toneMap.getNoteTracker().getTrack(noteListElement);
						if (track == null || track.getNumber() < 3 || ((track.getNumber() - 3) % 4 != 0)) {
							volume = 0;
						}
					}
					...
					if (volume > 0) {
						if (!voiceChannel3LastNotes.contains(note)) {
							...
							try {
								...
								if (writeTrack) {
									createEvent(voice1Track, voice3Channel, NOTEON, note, tick, volume);
								}
							} catch (InvalidMidiDataException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
						...
						if (voiceChannel3LastNotes.contains(note)) {
							...
							try {
								...
								if (writeTrack) {
									createEvent(voice1Track, voice3Channel, NOTEOFF, note, tick, volume);
								}
							} catch (InvalidMidiDataException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
				...
				for (ToneMapElement toneMapElement : ttfElements) {
					...
					if (noteListElement != null) {
						volume = getNoteVolume(lowVoiceThreshold, highVoiceThreshold, playLog, logFactor, playPeaks,
								toneMapElement.isPeak, noteListElement.maxAmp);
						NoteTrack track = toneMap.getNoteTracker().getTrack(noteListElement);
						if (track == null || track.getNumber() < 4 || ((track.getNumber() - 4) % 4 != 0)) {
							volume = 0;
						}
					}
					...
					if (volume > 0) {
						if (!voiceChannel4LastNotes.contains(note)) {
							...
							try {
								...
								if (writeTrack) {
									createEvent(voice1Track, voice4Channel, NOTEON, note, tick, volume);
								}
							} catch (InvalidMidiDataException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
						...
						if (voiceChannel4LastNotes.contains(note)) {
							...
							try {
								...
								if (writeTrack) {
									createEvent(voice1Track, voice4Channel, NOTEOFF, note, tick, volume);
								}
							} catch (InvalidMidiDataException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
					...
					if (noteListElement != null) {
						...
						if (playLog) {
							...
							if (amplitude > highVoiceThreshold) {
								...
								volume = (int) (((amplitude - lowVoiceThreshold)
										/ (highVoiceThreshold - lowVoiceThreshold)) * 127);
							}
						}
					}
		...
		private boolean playChordChannel2(MidiQueueMessage mqm) {
			...
			for (int note = 0; note < 12; note++) {
				...
				LOG.severe("MIDI C4: " + toneTimeFrame.getStartTime() + ", " + hasBeat + ", " + volume + ", " + note);
				if (volume > 0) {
					// octaveAdjust = toneTimeFrame.getOctave() ;
					octaveAdjust = 4;
					if (!chordsChannel2LastNotes.contains(note)) {
						midiMessage = new ShortMessage();
						try {
							LOG.severe("MIDI C4 NOTE_ON: " + toneTimeFrame.getStartTime() + ", " + hasBeat + ", "
									+ volume + ", " + note);
...
//						midiMessage = new ShortMessage();
//						try {
//							LOG.severe("MIDI C4 NOTE_OFF ON BEAT: " + toneTimeFrame.getStartTime() + ", " + hasBeat
//									+ ", " + volume + ", " + note);
...
//						midiMessage = new ShortMessage();
//						try {
//							LOG.severe("MIDI C4 NOTE_ON ON BEAT: " + toneTimeFrame.getStartTime() + ", " + hasBeat
//									+ ", " + volume + ", " + note);
//							midiMessage.setMessage(ShortMessage.NOTE_ON, chord2Channel.num, (note + 12 * octaveAdjust),
//									volume);
//							if (writeTrack) {
...
//						midiMessages.add(midiMessage);
					...
					if (chordsChannel2LastNotes.contains(note)) {
						midiMessage = new ShortMessage();
						try {
							LOG.severe("MIDI C4 NOTE_OFF: " + toneTimeFrame.getStartTime() + ", " + hasBeat + ", "
									+ volume + ", " + note);
							midiMessage.setMessage(ShortMessage.NOTE_OFF, chord2Channel.num, (note + 12 * octaveAdjust),
									0);
							if (writeTrack) {
		...
		private boolean playBeatChannel4(MidiQueueMessage mqm) {
			...
			int beat4Note = drum;
			boolean hasBeat = false;
			if (cm.getBeatBeforeTime(beatTimeFrame.getStartTime(), 110) != -1) {
				hasBeat = true;
				LOG.severe(">>MIDI C4 has beat: " + beatTimeFrame.getStartTime());
			}

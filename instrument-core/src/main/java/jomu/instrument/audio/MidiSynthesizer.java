...
public class MidiSynthesizer implements ToneMapConstants {
	...
	private static final Map<Integer, Integer> channelDeviceMap = Map.of(
			VOICE_1_CHANNEL, 0, VOICE_2_CHANNEL, 1, VOICE_3_CHANNEL, 2, VOICE_4_CHANNEL, 3,
			CHORD_1_CHANNEL, 4, CHORD_2_CHANNEL, 5, PAD_1_CHANNEL, 6, PAD_2_CHANNEL, 7, BASE_1_CHANNEL, 8,
			BEATS_CHANNEL, 9);
			...
			if (useSynthesizer && midiDevice != null) {
				...
				if (instruments == null || instruments.length == 0) {
					...
					if (sb != null) {
						...
						instruments = ((Synthesizer) midiSynthesizer).getDefaultSoundbank().getInstruments();
					} else {
		...
		private void switchOffSynthTracks(ToneTimeFrame toneTimeFrame, boolean silentWrite, boolean writeTrack) {
			...
			if (!silentWrite && midiDevice != null && midiDevice.isOpen()) {
				for (ShortMessage mm : midiMessages) {
					try {
						midiDevice.getReceiver().send(mm, -1);
		...
		private boolean playVoiceChannel1(MidiQueueMessage mqm) {
			...
			for (ToneMapElement toneMapElement : ttfElements) {
				...
				switch (noteStatusElement.state) {
					...
					case CONTINUING:
						LOG.severe(
								">>V1 MIDI NOTE CANDIDATE...: " + mqm.getSequence() + ", " + note + ", " + volume + ", "

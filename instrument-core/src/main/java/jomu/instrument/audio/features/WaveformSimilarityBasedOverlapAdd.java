...
public class WaveformSimilarityBasedOverlapAdd implements AudioProcessor {
	...
	private void applyNewParameters() {
		...
		// pMidBuffer and pRefBuffer are initialized with 8 times the needed length to
		// prevent a reset
	 ...
	 * @param postion     The position where to start the seek operation, in the
	 *                    input buffer.
	...
	/**
	 * Slopes the amplitude of the 'midBuffer' samples so that cross correlation is
	 * faster to calculate. Why is this faster?
	 */
	...
	public static class Parameters {
		...
		/**
		 * @param tempo           The tempo change 1.0 means unchanged, 2.0 is + 100% ,
		 ...
		 *                        milliseconds. This determines to how long sequences
		 *                        the original sound is chopped in the time-stretch
		 *                        algorithm.
		 * 
		 *                        The larger this value is, the lesser sequences are
		 *                        used in processing. In principle a bigger value sounds
		 *                        better when slowing down tempo, but worse when
		 *                        increasing tempo and vice versa.
		 * 
		 *                        Increasing this value reduces computational burden &
		 *                        vice versa.
		 * @param newSeekWindowMs Seeking window length in milliseconds for algorithm
		 *                        that finds the best possible overlapping location.
		 *                        This determines from how wide window the algorithm may
		 *                        look for an optimal joining location when mixing the
		 *                        sound sequences back together.
		 * 
		 *                        The bigger this window setting is, the higher the
		 *                        possibility to find a better mixing position will
		 *                        become, but at the same time large values may cause a
		 *                        "drifting" artifact because consequent sequences will
		 *                        be taken at more uneven intervals.
		 * 
		 *                        If there's a disturbing artifact that sounds as if a
		 *                        constant frequency was drifting around, try reducing
		 *                        this setting.
		 * 
		 ...
		 *                        sequences are mixed back together, to form a
		 *                        continuous sound stream, this parameter defines over
		 *                        how long period the two consecutive sequences are let
		 *                        to overlap each other.
		 * 
		 *                        This shouldn't be that critical parameter. If you
		 *                        reduce the DEFAULT_SEQUENCE_MS setting by a large
		 *                        amount, you might wish to try a smaller value on this.
		 * 
		 *                        Increasing this value increases computational burden &
		 *                        vice versa.
		 */
